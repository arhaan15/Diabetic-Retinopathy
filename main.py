...
model.load_state_dict(torch.load('best_dr_fixed_model.pth', map_location=DEVICE, weights_only=True))
model.eval()
...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            grade = int(probs.argmax(1).item())
            confidence = float(probs[0][grade].item())
        return {"grade": grade, "label": DR_LABELS[grade], "confidence": confidence}
...
async def gradcam(file: UploadFile = File(...)):
    ...
    try:
        ...
        h2 = target_layer.register_backward_hook(save_grad)
        ...
        orig = np.frombuffer(contents, np.uint8)
        orig = cv2.imdecode(orig, cv2.IMREAD_COLOR)
